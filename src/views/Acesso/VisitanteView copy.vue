        ...
        async alterarLocal() {
            ...
            if (this.localSelecionado !== null) {
                try {
                    ...
                    console.error('Error fetching acessos data:', error);
        ...
        salvarVisita() {
            ...
            axios.post(urlVisita, this.dadosVisitaModal)
                .then(response => {
                    ...
                    if (response.data.cod === 1) { //código 1 erro de qrcode invalido
                    ...
                    this.qrCodeCartao = ''; //limpa qrcode lido

                    } else {
                        // SALVA A VISITA
                        if (response.data.msg === "A pessoa não possui um endereço de email válido. O email não foi enviado.") {

                       
                           toaster.show(`QR Code não será enviado por e-mail pois o visitante não possui e-mail cadastrado. `, { type: "warning" });

                        } else if (this.qrCodeCartao) {

                            toaster.show(`Visita cadastrada com catão de QR Code`, { type: "success" });

                        } else {

                             toaster.show(`Visita cadastrada. QR Code enviado por e-mail`, { type: "success" });
